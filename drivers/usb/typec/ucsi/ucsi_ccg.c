...
static void ccg_drain_responses(struct ucsi_ccg *uc)
{
        ...
        while (lim--) {
                ...
                intr = BIT(0);
                ccg_write(uc, HPI_ADDR_INTR_REG, &intr, sizeof(intr));
                                            ...
                                            unsigned int timeout_ms)
{
        ...
        int ret = ccg_wait_async_event(uc, &ev0, &ev1, timeout_ms);
                ...
                dev_err(uc->dev,
                         "AKdiag: %s: success (resp0=0x%02x resp1=0x%02x)\n",
...
static int get_fw_info(struct ucsi_ccg *uc)
{
	...
	dev_err(uc->dev, "AK:ccg device info: fw_version:%u mode:%u bl_mode:%u silicon_id:0x%04x bl_last_row:0x%04x\n",
...
static int do_flash(struct ucsi_ccg *uc, enum enum_flash_mode mode)
{
...

	{
		...
		err = ccg_enter_flashing_robust(uc, 0);
		if (err) {
...
static int ucsi_ccg_probe(struct i2c_client *client)
{
	...
	if (!uc->fw_version) {
		...
		dev_info(uc->dev, "fw_version is empty, flashing firmware in probe\n");

		status = ccg_fw_update_needed(uc, &flash_mode);
		if (status < 0) {
			dev_err(uc->dev, "ccg_fw_update_needed failed - %d\n", status);
			return status;
		}

		if (flash_mode != FLASH_NOT_NEEDED) {
			status = ccg_fw_update(uc, flash_mode);
			if (status < 0) {
				dev_err(uc->dev, "ccg_fw_update failed - %d\n", status);
				return status;
			}

			/* do_flash() resets the device, reinitialize ucsi control */
			status = ucsi_ccg_init(uc);
			if (status < 0) {
				dev_err(uc->dev, "ucsi_ccg_init failed after fw update - %d\n", status);
				return status;
			}

			status = get_fw_info(uc);
			if (status < 0) {
				dev_err(uc->dev, "get_fw_info failed after fw update - %d\n", status);
				return status;
			}
		}
	}
